...
export function SiteHeader() {
  ...
  return <header className="site-header">
    ...
    <nav className={open ? "primary-nav open" : "primary-nav"} aria-label="Primary navigation" id="primary-navigation">
      ...
      <Link className="mobile-only-link" href="/login">Log in</Link>
    </nav>
    <div className="header-actions"><Link className="text-link" href="/login">Log in</Link><Link className="button button-small button-primary" href="/trial">Request access</Link><button className="menu-toggle" type="button" aria-label="Toggle navigation" aria-controls="primary-navigation" aria-expanded={open} onClick={() => setOpen(value => !value)}><span/><span/><span/></button></div>
  </header>;
}
